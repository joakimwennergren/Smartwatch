 ...
 */
    ...
    (!defined(__DA14531__)) ||                                                                  \
    ((defined (__DA14531_01__) || defined (__DA14535__)) && defined (__EXCLUDE_ROM_APP_TASK__))
/**
 ****************************************************************************************
                                ...
                                ke_task_id_t const src_id)
{
    switch(param->operation)
    {
        ...
        default:
        {
            ...
            else if ((param->operation == GAPC_GET_CON_RSSI) && (param->status == GAP_ERR_COMMAND_DISALLOWED))
            {
                // Application can receive from the ble stack the following allowed
                // message with parameters:
                // param->operation = GAPC_GET_CON_RSSI
                // param->status    = GAP_ERR_COMMAND_DISALLOWED
                // This can happen when an rssi request is active and a disconnection occurs 
                // In that case the message can be ignored instead of ending up to
...
#if defined(__DA14531__) && BOND_CMD_SUPPORTED
...
#if defined(USE_AT_RSSI) && (defined(__DA14585__) || defined(__DA14586__))
#include "user_codeless_env.h"

/*
 ****************************************************************************************
 * @brief Handles GAPC_CON_RSSI_IND message.
 * @param[in] msgid     Id of the message received.
 * @param[in] param     Pointer to the parameters of the message.
 * @param[in] dest_id   ID of the receiving task instance.
 * @param[in] src_id    ID of the sending task instance.
 ...
 ****************************************************************************************
*/
static int gapc_con_rssi_ind_handler(ke_msg_id_t const msgid,struct gapc_con_rssi_ind const *param,
    ke_task_id_t const dest_id,ke_task_id_t const src_id)
{
    codeless_env.rssi = param->rssi;
    return (KE_MSG_CONSUMED);
}
...
#if defined (__DA14531__) && (!defined (__DA14531_01__) && !defined (__DA14535__))
#if !defined(__EXCLUDE_ROM_APP_TASK__)
...
static const struct ke_msg_handler app_gap_process_handlers[]=
{
    ...
    {GATTC_SVC_CHANGED_CFG_IND,             (ke_msg_func_t)gattc_svc_changed_cfg_ind_handler_ROM},
    {GAPC_PEER_FEATURES_IND,                (ke_msg_func_t)gapc_peer_features_ind_handler_ROM},

#if (BLE_APP_SEC)
    #if BOND_CMD_SUPPORTED
    {GAPC_SECURITY_IND,                     (ke_msg_func_t)gapc_security_ind_handler},
    #else
    {GAPC_SECURITY_IND,                     (ke_msg_func_t)gapc_security_ind_handler_ROM},
    #endif
#endif
};
#endif
#endif // Only for using DA14531 and ROM app_task.c
...
#if defined(USE_AT_RSSI) && defined(__DA14535__)

extern int gapm_device_ready_ind_handler_535_ROM(ke_msg_id_t const msgid,
                                         void const *param,
                                         ke_task_id_t const dest_id,
                                         ke_task_id_t const src_id);

extern int gapm_cmp_evt_handler_535_ROM(ke_msg_id_t const msgid,
                                struct gapm_cmp_evt const *param,
                                ke_task_id_t const dest_id,
                                ke_task_id_t const src_id);

extern int gapc_cmp_evt_handler_535_ROM(ke_msg_id_t const msgid,
                                struct gapc_cmp_evt const *param,
                                ke_task_id_t const dest_id,
                                ke_task_id_t const src_id);

extern int gapc_connection_req_ind_handler_535_ROM(ke_msg_id_t const msgid,
                                           struct gapc_connection_req_ind const *param,
                                           ke_task_id_t const dest_id,
                                           ke_task_id_t const src_id);
                                           
extern int gapc_disconnect_ind_handler_535_ROM(ke_msg_id_t const msgid,
                                       struct gapc_disconnect_ind const *param,
                                       ke_task_id_t const dest_id,
                                       ke_task_id_t const src_id);

extern int gapc_get_dev_info_req_ind_handler_535_ROM(ke_msg_id_t msgid,
                                             struct gapc_get_dev_info_req_ind *param,
                                             ke_task_id_t const dest_id,
                                             ke_task_id_t const src_id);

extern int gapc_set_dev_info_req_ind_handler_535_ROM(ke_msg_id_t msgid,
                                             struct gapc_set_dev_info_req_ind *param,
                                             ke_task_id_t const dest_id,
                                             ke_task_id_t const src_id);

extern int gapm_profile_added_ind_handler_535_ROM(ke_msg_id_t msgid,
                                          struct gapm_profile_added_ind *param,
                                          ke_task_id_t const dest_id,
                                          ke_task_id_t const src_id);

extern int gapm_adv_report_ind_handler_535_ROM(ke_msg_id_t msgid,
                                       struct gapm_adv_report_ind *param,
                                       ke_task_id_t dest_id,
                                       ke_task_id_t src_id);

extern int gapc_param_update_req_ind_handler_535_ROM(ke_msg_id_t msgid,
                                             struct gapc_param_update_req_ind *param,
                                             ke_task_id_t const dest_id,
                                             ke_task_id_t const src_id);

extern int gapc_le_pkt_size_ind_handler_535_ROM(ke_msg_id_t msgid,
                                        struct gapc_le_pkt_size_ind *param,
                                        ke_task_id_t const dest_id,
                                        ke_task_id_t const src_id);

extern int gattc_svc_changed_cfg_ind_handler_535_ROM(ke_msg_id_t msgid,
                                             struct gattc_svc_changed_cfg *param,
                                             ke_task_id_t const dest_id,
                                             ke_task_id_t const src_id);

extern int gapc_peer_features_ind_handler_535_ROM(ke_msg_id_t msgid,
                                          struct gapc_peer_features_ind const *param,
                                          ke_task_id_t const dest_id,
                                          ke_task_id_t const src_id);


extern int gapc_security_ind_handler_535_ROM(ke_msg_id_t msgid,
                                     struct gapc_security_ind *param,
                                     ke_task_id_t const dest_id,
                                     ke_task_id_t const src_id);
...
static const struct ke_msg_handler app_gap_process_handlers[]=
{
    {GAPM_DEVICE_READY_IND,                 (ke_msg_func_t)gapm_device_ready_ind_handler_535_ROM},
    {GAPM_CMP_EVT,                          (ke_msg_func_t)gapm_cmp_evt_handler_535_ROM},
    {GAPC_CMP_EVT,                          (ke_msg_func_t)gapc_cmp_evt_handler_535_ROM},
    {GAPC_CONNECTION_REQ_IND,               (ke_msg_func_t)gapc_connection_req_ind_handler_535_ROM},
    {GAPC_DISCONNECT_IND,                   (ke_msg_func_t)gapc_disconnect_ind_handler_535_ROM},
    {GAPC_GET_DEV_INFO_REQ_IND,             (ke_msg_func_t)gapc_get_dev_info_req_ind_handler_535_ROM},
    {GAPC_SET_DEV_INFO_REQ_IND,             (ke_msg_func_t)gapc_set_dev_info_req_ind_handler_535_ROM},
    {GAPM_PROFILE_ADDED_IND,                (ke_msg_func_t)gapm_profile_added_ind_handler_535_ROM},
    {GAPM_ADV_REPORT_IND,                   (ke_msg_func_t)gapm_adv_report_ind_handler_535_ROM},
    {GAPC_PARAM_UPDATE_REQ_IND,             (ke_msg_func_t)gapc_param_update_req_ind_handler_535_ROM},
    {GAPC_LE_PKT_SIZE_IND,                  (ke_msg_func_t)gapc_le_pkt_size_ind_handler_535_ROM},
    {GATTC_SVC_CHANGED_CFG_IND,             (ke_msg_func_t)gattc_svc_changed_cfg_ind_handler_535_ROM},
    {GAPC_PEER_FEATURES_IND,                (ke_msg_func_t)gapc_peer_features_ind_handler_535_ROM},
...
#else
#if (BLE_APP_SEC)
    {GAPC_SECURITY_IND,                     (ke_msg_func_t)gapc_security_ind_handler_535_ROM},
#endif
...
#if defined(USE_AT_RSSI) && defined(__DA14531__)
#include "user_codeless_env.h"

enum process_event_response app_gap_process_handler_with_rssi (ke_msg_id_t const msgid,
                                                     void const *param,
                                                     ke_task_id_t const dest_id,
                                                     ke_task_id_t const src_id,
                                                     enum ke_msg_status_tag *msg_ret)
{
    ...
    switch (msgid)
    {
        ...
        {
            // Handling of the RSSI indication 
            codeless_env.rssi = ((struct gapc_con_rssi_ind*)param)->rssi;
...
#endif // USE_AT_RSSI && __DA14531__

#if defined(__DA14531__) && !defined(__DA14531_01__) && !defined(__DA14535__) && defined(CFG_DATAPMP)
enum process_event_response app_gap_process_handler_with_bkpt_bypass (ke_msg_id_t const msgid,
                                                     void const *param,
                                                     ke_task_id_t const dest_id,
                                                     ke_task_id_t const src_id,
                                                     enum ke_msg_status_tag *msg_ret)
{
    switch (msgid)
    {
        ...
        {
            // Handling of the RSSI indication 
            volatile uint8_t operation = ((struct gapc_cmp_evt*)param)->operation;
            volatile uint8_t status = ((struct gapc_cmp_evt*)param)->status;
