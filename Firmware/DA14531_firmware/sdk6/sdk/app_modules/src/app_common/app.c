...
#if (BLE_GL_SENSOR)
    {TASK_ID_GLPS,          app_glps_create_db, app_glps_enable},
#endif

#if (BLE_PLX_SERVER)
    {TASK_ID_PLXS,          app_plxs_create_db, app_plxs_enable},
#endif

#if (BLE_CGM_SERVER)
    {TASK_ID_CGMS,          app_cgms_create_db, app_cgms_enable},
#endif
...
/**
 ****************************************************************************************
 * @brief Create advertising message 
 * @return gapm_start_advertise_cmd     Pointer to the advertising message
 * @note This function can save some space as the code it contains was used previously 
 *       in multiple functions
 ****************************************************************************************
 */
static struct gapm_start_advertise_cmd *app_easy_gap_create_fill_advertise_msg(bool undirected)
{
    ...
    if (USER_DEVICE_NAME_LEN > 0)
    {
        ...
        uint16_t total_adv_space = (undirected ? 3 : 0) + USER_ADVERTISE_DATA_LEN + 2 + USER_DEVICE_NAME_LEN;
        // Get remaining space in the Scan Response Data ( plus 2 bytes are used for the length and flag bytes of the Device Name)
        uint16_t total_scan_space = USER_ADVERTISE_SCAN_RESPONSE_DATA_LEN + 2 + USER_DEVICE_NAME_LEN;

        if (total_adv_space <= ADV_DATA_LEN)
        {
            append_device_name(&cmd->info.host.adv_data_len,
                               USER_DEVICE_NAME_LEN,
                               &(cmd->info.host.adv_data[USER_ADVERTISE_DATA_LEN]),
                               USER_DEVICE_NAME);
        }
        else if (total_scan_space <= SCAN_RSP_DATA_LEN)
        {
            append_device_name(&cmd->info.host.scan_rsp_data_len,
                               USER_DEVICE_NAME_LEN,
                               &(cmd->info.host.scan_rsp_data[USER_ADVERTISE_SCAN_RESPONSE_DATA_LEN]),
                               USER_DEVICE_NAME);
         }
    }
...
static struct gapm_start_advertise_cmd* app_easy_gap_non_connectable_advertise_start_create_msg(void)
{
    ...
    if (adv_cmd == NULL)
    {
        ...
        adv_cmd = app_easy_gap_create_fill_advertise_msg(false);
...
static struct gapm_start_advertise_cmd* app_easy_gap_undirected_advertise_start_create_msg(void)
{
    ...
    if (adv_cmd == NULL)
    {
        ...
        adv_cmd = app_easy_gap_create_fill_advertise_msg(true);

        adv_cmd->op.code = GAPM_ADV_UNDIRECT;

        if (user_adv_conf.adv_filt_policy == ADV_ALLOW_SCAN_ANY_CON_WLST)
        {
            adv_cmd->info.host.mode = GAP_NON_DISCOVERABLE;
        }
        ...
        memcpy(adv_cmd->info.host.peer_info.addr.addr, &(gapm_env.addr), BD_ADDR_LEN * sizeof(uint8_t));
        adv_cmd->info.host.peer_info.addr_type = ((GAPM_F_GET(gapm_env.cfg_flags, ADDR_TYPE) == GAPM_CFG_ADDR_PUBLIC) ? ADDR_PUBLIC : ADDR_RAND);
        ...
        for (uint8_t bdb_slot = 0; bdb_slot < bdb_size; bdb_slot++)
        {
            ...
            if (bonded_dev_found)
            {
                ...
                memcpy(adv_cmd->info.host.peer_info.addr.addr, &bonded_dev_info.addr, BD_ADDR_LEN * sizeof(uint8_t));
                adv_cmd->info.host.peer_info.addr_type = bonded_dev_info.addr_type;
                break;
            }
        }
#endif
#endif // not applicable to DA14531_01 or DA14535
    }
    return adv_cmd;
}
...
void app_easy_gap_param_update_set_active(uint8_t conidx)
{
...
#if defined(CODELESS_585)
void app_easy_reset_config_cmd(void)
{
...
#if defined (__DA14531_01__) || !defined (__DA14531__) || defined (__EXCLUDE_ROM_APP_TASK__)
...
#endif // use always from SDK for DA14585/586, DA14531-01
...
void app_gap_get_con_rssi(uint8_t conidx)
{
    struct gapc_get_info_cmd *cmd = KE_MSG_ALLOC(GAPC_GET_INFO_CMD,KE_BUILD_ID(TASK_GAPC, conidx),
        TASK_APP, gapc_get_info_cmd);
