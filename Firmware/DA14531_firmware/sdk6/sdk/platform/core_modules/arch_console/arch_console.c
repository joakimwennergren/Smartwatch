...
#include "uart.h"
    ...
    #ifdef CFG_UART2_DISABLE
        #error "UART2 cannot be used for UART Console Print because it has beed disabled"
    #endif
    #define UART        UART2
...
printf_msg *printf_msg_list      __SECTION_ZERO("retention_mem_area0");
bool defer_sending               __SECTION_ZERO("retention_mem_area0");
...
#if defined (__DA14535__) && !defined (__EXCLUDE_ROM_ARCH_CONSOLE__)
__STATIC_INLINE int _putc(char **wr_ptr, char *buf, uint32_t length, char ch)
{
...
__STATIC_INLINE int _puts(char **wr_ptr, char *buf, uint32_t length, char *s, uint32_t str_len)
{
    uint32_t i;
...
uint32_t arch_strlen(const char *s)
{
...
int32_t arch_vsnprintf(char *buffer, uint32_t buffer_len, const char *fmt, va_list va)
{
    char *pbuffer = buffer;
    char bf[24];
    char ch;
...
int32_t arch_snprintf(char* buffer, uint32_t buffer_len, const char *fmt, va_list va)
{
    int ret;
...
int arch_sprintf(char *str, const char *fmt, ...)
{
    va_list args;
    char my_buf[PRINT_SZ];

    va_start(args, fmt);
    arch_vsnprintf(my_buf, sizeof(my_buf), fmt, args);
    va_end(args);
