...
#if (defined (__DA14531_01__) || defined (__DA14535__)) && !defined (__EXCLUDE_ROM_BLE_ARP__)
...
bool rf_ldo_calibration_pending     __SECTION_ZERO("retention_mem_area0");
uint8_t rf_ldo_init_value           __SECTION_ZERO("retention_mem_area0");
#endif
...
static uint8_t rf_rssi_convert (uint8_t rssi_reg)
{
     ...
     * DA14531: The equation to use for Input Power = 0.498 * RSSI - 127,
     * DA14535: The equation to use for Input Power = 0.491 * RSSI - 126,
    ...
    */
    ...
    return (uint8_t) ((uint8_t) (-126) + (uint8_t) ((491 * RSSI_Value_temp) / 1000));
#else
...
static uint8_t rf_txpwr_dbm_get(uint8_t conidx, uint8_t req_pwr_level)
{
    ...
    else
    {
        ...
        {
            tx_pow_lvl = RF_TX_PWR_LVL_MAX_DBM;
        }
        else
        {
#if defined (CFG_ENHANCED_TX_PWR_CTRL)
...
#if defined(RF_OPT)
static void rf_activate_and_configure_radio(void)
{
...
SYS_INIT_INTERIM_RUN_ONCE_CODE void rf_init_func(struct rwip_rf_api *api);
...
void rf_init_func(struct rwip_rf_api *api)
{
    ...
    {
        rf_pa_pwr_set(RF_TX_PWR_LVL_MIN_DBM);
    }
#else
    // Initialize Tx output power level to default 0dBm
...
#if defined(RF_OPT)
     rf_activate_and_configure_radio();
#else
    set_recommended_settings();          // Preferred settings
...
#if (USE_RF_LDO_CALIBRATION)
    rf_ldo_calibration_pending = true;
    rf_ldo_init_value = GetBits16(RF_LDO_CTRL_REG, LDO_RADIO_LEVEL);
    rf_ldo_calibration();
#endif
 ...
 ***/
SYS_INIT_INTERIM_RUN_ONCE_CODE void ble_init_arp_func(uint32_t base_addr);
...
#if defined (__DA14535__)
    // Re-initialize table entry since the ROM `arp_init_table` variable was built with the DA14531 dBm enumeration
    arp_table->arpt_pref[ARPT_RF_ATTR_REG_IDX].bf.data = RF_TX_PWR_LVL_0d0 << 8;
#endif
#else
...
static void update_pwr(rf_pwr_mode_t pwr_mode, uint8_t conidx)
{
    ...
    // Select between 0dBm and 3dBm settings
    ...
    else
    {
        ...
        {
            // Near Field Mode Enable
            arp_table->arpt_pref[ARPT_RF_ATTR_REG_IDX].bf.data = RF_TX_PWR_LVL_MIN_DBM << 8;
        }
        else
        {
            // Load user radio power settings depending on the detected mode
