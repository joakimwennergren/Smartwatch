...
extern void JT_smpc_pdu_recv_func(uint8_t conidx, struct l2cc_pdu *pdu);
#else
...
#if defined (__DA14531_01__) || defined (__DA14535__) || defined(__DA14531__)
...
#else
extern uint8_t JT_l2cc_pdu_unpack_func(struct l2cc_pdu *p_pdu, uint16_t *p_offset, uint16_t *p_rem_len,
                                    const uint8_t *p_buffer, uint16_t pkt_length, uint8_t conidx, uint8_t llid);
#endif
...
extern int l2cc_pdu_recv_ind_handler_func(ke_msg_id_t const msgid, struct l2cc_pdu_recv_ind *ind,
                                          ke_task_id_t const dest_id, ke_task_id_t const src_id);
#else
extern int JT_l2cc_pdu_recv_ind_handler_func(ke_msg_id_t const msgid, struct l2cc_pdu_recv_ind *ind,
                                          ke_task_id_t const dest_id, ke_task_id_t const src_id);
#endif
...
#else
extern bool JT_llc_le_length_effective_func(uint16_t conhdl);
#endif
...
extern void llm_util_aa_gen_func(uint8_t *acc_addr);
extern void JT_llm_le_adv_report_ind_func(struct co_buf_rx_desc *rxdesc);
#endif
/*
 * Heaps
 ****************************************************************************************
...
// The "smpm_ecdh_key_create_empty_func()" cannot be used in DA14531-01 or DA14535
#if !defined (__DA14531_01__) && !defined (__DA14535__)
#if (!ENABLE_SMP_SECURE)
void smpm_ecdh_key_create_empty_func(void)
{
}
...
/**
 ****************************************************************************************
 * @brief Send dummy message to save power if legacy pairing is used in DA14531-01, DA14535.
 * @param[in] msg       Pointer to message
 ****************************************************************************************
 */
void llm_p256_start_dummy_func(struct ke_msg *msg)
{
...
__USED ke_task_id_t _prf_get_id_from_task_func(ke_msg_id_t task)
{
    uint8_t idx = KE_IDX_GET(task);
    task = KE_TYPE_GET(task);
    
    if(task == TASK_FSP_EXT) {
        return KE_BUILD_ID(TASK_ID_FSP_EXT, idx);
    }

    return prf_get_id_from_task_func(task);
}

__USED ke_task_id_t _prf_get_task_from_id_func(ke_msg_id_t id)
{
    uint8_t idx = KE_IDX_GET(id);
    id = KE_TYPE_GET(id);

    if(id == TASK_ID_FSP_EXT) {
        return KE_BUILD_ID(TASK_FSP_EXT, idx);
    }
    
    return prf_get_task_from_id_func(id);
}
...
#if USE_START_SMP_TIMEOUT_TIMER_UPON_TX_SEC_REQ_CMD  // Patch applies to DA14585/586 only
    (void *) JT_smpc_pdu_recv_func,
#else
...
#ifdef CFG_FSP_EXT_TASK
    (void *) _prf_get_id_from_task_func,
    (void *) _prf_get_task_from_id_func,
#else
...
#else
    (void *) JT_l2cc_pdu_recv_ind_handler_func,
#endif
...
#ifndef __DA14531_01__
#ifdef __DA14535__
    (void *) JT_llm_le_adv_report_ind_func,
#else
    (void *) llm_le_adv_report_ind_func,                // No central support in DA14531-01
#endif
#endif
    (void *) PK_PointMult_func,
#if defined (CFG_HCI) && !defined (__DA14531__)
    (void *) JT_llm_p256_start_func,
#elif (!ENABLE_SMP_SECURE) && (defined (__DA14531_01__) || defined (__DA14535__))   // save power when legacy pairing is used (only for DA14531-01, DA14535)
    (void *) llm_p256_start_dummy_func,
#else
...
#else
    (void *) JT_llc_le_length_effective_func,
#endif
...
/*
 * DA14531-01, DA14535 BLE stack behavior:
 *  The ECDH keys are always created after a pairing request, either legacy
 *  pairing or either secure connections has been selected.
 */
#if (ENABLE_SMP_SECURE) || (defined (__DA14531_01__) || defined (__DA14535__))
    (void *) smpm_ecdh_key_create_func,
#else
    (void *) smpm_ecdh_key_create_empty_func,
#endif // ENABLE_SMP_SECURE
#if defined (__DA14531__)
    (void *) ble_init_arp_func,
#endif
#if defined (__DA14535__)       // Only for DA14535AB silicon
    (void *) &llm_util_aa_gen_func,
#endif
    NULL,
    NULL,
    NULL,
...
#pragma diag_warning 1296
...
#if defined (__DA14531__)
    460,    //lld_evt_abort_cnt_duration
#else
    ...
    12,     //sw_version_major_pos (v5.3)
#elif defined (__DA14531__)
    10,     //sw_version_major_pos (v5.1)
#endif
#if defined (__DA14531__)
...
#pragma diag_suppress 1296
    ...
    (uint32_t) NULL,
    (uint32_t) NULL,
    350,                                    // uart_char_delay_iterations_pos
#endif
