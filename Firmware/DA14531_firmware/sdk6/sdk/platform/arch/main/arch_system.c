...
extern bool rf_ldo_calibration_pending;
#endif

#ifndef CFG_CALIBRATION_TEMP_DIFF
#define CFG_CALIBRATION_TEMP_DIFF           (8)
#endif

#if (USE_XTAL32M_DYN_FREQ_TRIMMING)
#ifndef CFG_XTAL32M_TRIM_TEMP_DIFF
#define CFG_XTAL32M_TRIM_TEMP_DIFF          (5)
#endif
#endif
#endif
...
#if defined (__NON_BLE_EXAMPLE__)
#if (defined (__DA14531_01__) || defined (__DA14535__)) && !defined (__EXCLUDE_ROM_ARCH_SYSTEM__)
extern void set_xtal32m_trim_value_ROM(uint16_t trim_value);
#endif
#endif
...
volatile uint32_t startup_sleep_delay = STARTUP_SLEEP_DELAY_DEFAULT;

#if defined (__DA14531__)
/// Last temperature read from the die temperature sensor through GPADC
int8_t rf_cal_last_temp    __SECTION_ZERO("retention_mem_area0");
/// Radio calibration counter increasing by one just before calibration runs
static uint32_t rf_cal_count __SECTION_ZERO("retention_mem_area0");
#if (USE_XTAL32M_DYN_FREQ_TRIMMING)
/* 
 Note: The values in the tables do not depend on the ICs but on the crystals(XTAL32M) connected to the ICs.
 Therefore if the crystal changes, the values below should change as well. Please refer to the schematics of the 
 DA14533/DA14535 daughterboards to view the exact part number of the crystals, for which these values have been calculated.
*/
#if defined(__DA14533__)
const int16_t trimfit_dt[] = {2189, 2923, 2773, 1906, 643, -668, -1678, -2431, -2820, -2833, -2274, -958, 1933, 6586};
static const int8_t trim_delta = 5;
static const int16_t adc_min = -140;
#elif defined(__DA14535__)
static const int16_t trimfit_dt[] = {-429, 89, 494, 777, 853, 791, 647, 439, 213, -21, -259, -501, -750, -982, -1179, -1321, -1310, -1158, -855, -386, 319, 1305, 2497}; 
static const int8_t trim_delta = 4;
static const int16_t adc_min = -149;
#endif // __DA14533__
int8_t xtal32m_trim_last_temp   __SECTION_ZERO("retention_mem_area0");
#endif
#else
uint16_t last_temp_count    __SECTION_ZERO("retention_mem_area0"); // temperature counter
#endif

uint32_t last_time          __SECTION_ZERO("retention_mem_area0"); // time of last temperature count measurement
...
extern void rf_recalibration(void);
...
rcx_time_data_t rcx_time_data       __SECTION_ZERO("retention_mem_area0");

#if (RCX_MEASURE_ENABLED)
rcx_stats_data_t rcx_stats_data     __SECTION_ZERO("retention_mem_area0");
#endif
...
uint32_t otp_hdr_timestamp = 0;
#endif

// Required DA14531-01, DA14535 ROM symbols
#if (defined (__DA14531_01__) || defined (__DA14535__)) && !defined (__EXCLUDE_ROM_ARCH_SYSTEM__)
extern void init_pwr_and_clk_ble_part_start_ROM(void);
extern void init_pwr_and_clk_ble_part_xtal32k_ROM(void);
extern void init_pwr_and_clk_ble_part_rcx_ROM(void);
extern void init_pwr_and_clk_ble_part_end_ROM(void);
extern void calc_rcx_freq_ROM(rcx_time_data_t *ptr_rcx_time_data);
extern void calc_rcx_freq_stas_ROM(rcx_time_data_t *ptr_rcx_time_data, rcx_stats_data_t *ptr_rcx_stats_data);
extern uint8_t check_sys_startup_period_ROM(bool *ptr_sys_startup_flag, uint32_t startup_sleep_delay);
extern void set_xtal32m_trim_value_ROM(uint16_t trim_value);
#endif
 ...
 ****************************************************************************************
 */
static void select_lp_clk(void)
{
...
{
    init_pwr_and_clk_ble_part_start_ROM();

    select_lp_clk();

    if (arch_clk_is_XTAL32())
    {
        init_pwr_and_clk_ble_part_xtal32k_ROM();
    }
    else if (arch_clk_is_RCX20())
    {
        init_pwr_and_clk_ble_part_rcx_ROM();
#if defined (__DA14535__)
        SetBits16(CLK_RCX_REG, RCX_BIAS, 0xE);
#endif
    }
    else
    {
    ...
    init_pwr_and_clk_ble_part_end_ROM();
}
#else
void init_pwr_and_clk_ble(void)
{
...
#if defined (__DA14531__)
#if !defined (__DA14535__)
    SetBits16(GP_DATA_REG, P03_P04_FILT_DIS, 1);    // Disable RC input filter for P0_3 and P0_4
#endif
    ...
    else if ( arch_clk_is_RCX20( ) )
    {
        ...
        SetBits16(CLK_RCX_REG, RCX_BIAS, 0xE);
    #else
        SetBits16(CLK_RCX_REG, RCX_BIAS, 0xB);
    #endif
...
#endif // __EXCLUDE_ROM_ARCH_SYSTEM__
...
void read_rcx_freq(bool wait_for_calibration)
{
    if (rcx_cal_in_progress && ((CFG_LP_CLK == LP_CLK_FROM_OTP) || (CFG_LP_CLK == LP_CLK_RCX20)))
    {
...
#if (defined (__DA14531_01__) || defined (__DA14535__)) && !defined (__EXCLUDE_ROM_ARCH_SYSTEM__)
        calc_rcx_freq_ROM(&rcx_time_data);
#else
        ...
        rcx_time_data.rcx_period = (((uint32_t) ((value * RCX_PERIOD_FACTOR) >> 14) + 1) >> 1);
        // Calculate RCX slot duration
        rcx_time_data.rcx_slot_duration = (RCX_SLOT_DURATION_FACTOR / value);
        // Calculate RCX frequency. (1000000 / 625 >> 20) is equal to (100 >> 16)
        rcx_time_data.rcx_freq = (((uint32_t) rcx_time_data.rcx_slot_duration) * 100) >> 16;
#endif // __EXCLUDE_ROM_ARCH_SYSTEM__

#if (RCX_MEASURE_ENABLED)
#if (defined (__DA14531_01__) || defined (__DA14535__)) && !defined (__EXCLUDE_ROM_ARCH_SYSTEM__)
        calc_rcx_freq_stas_ROM(&rcx_time_data, &rcx_stats_data)
#else
        if (rcx_stats_data.rcx_period_last)
        {
            int diff = rcx_stats_data.rcx_period_last - rcx_time_data.rcx_period;
            if (abs(diff) > rcx_stats_data.rcx_period_diff)
            {
                rcx_stats_data.rcx_period_diff = abs(diff);
            }
        }
        rcx_stats_data.rcx_period_last = rcx_time_data.rcx_period;

        if (rcx_stats_data.rcx_freq_min == 0)
        {
            rcx_stats_data.rcx_freq_min = rcx_time_data.rcx_freq;
            rcx_stats_data.rcx_freq_max = rcx_time_data.rcx_freq;
        }

        if (rcx_time_data.rcx_freq < rcx_stats_data.rcx_freq_min)
        {
            rcx_stats_data.rcx_freq_min = rcx_time_data.rcx_freq;
        }
        else if (rcx_time_data.rcx_freq > rcx_stats_data.rcx_freq_max)
        {
            rcx_stats_data.rcx_freq_max = rcx_time_data.rcx_freq;
        }
#endif // __EXCLUDE_ROM_ARCH_SYSTEM__
#endif // RCX_MEASURE_ENABLED
    }
}
...
void read_rcx_freq(uint16_t cal_time)
{
    if ( (rcx_cal_in_progress) && ((CFG_LP_CLK == LP_CLK_FROM_OTP) || (CFG_LP_CLK == LP_CLK_RCX20)) )
    {
        ...
        rcx_time_data.rcx_freq = f;

#ifdef RCX_PERIOD_CORRECTION_FACTOR_IN_PPM
        rcx_time_data.rcx_period = ((uint64_t)64 * value) * (1000000 + RCX_PERIOD_CORRECTION_FACTOR_IN_PPM) / 1000000 / cal_time;
#else
        rcx_time_data.rcx_period = (64 * value) / cal_time;
#endif

        rcx_time_data.rcx_slot_duration = 625 * cal_time * 16;

#ifdef RCX_PERIOD_CORRECTION_FACTOR_IN_PPM
        rcx_time_data.rcx_slot_duration = (rcx_time_data.rcx_slot_duration << 20) * 1000000 / (1000000 + RCX_PERIOD_CORRECTION_FACTOR_IN_PPM);
#else
        rcx_time_data.rcx_slot_duration = rcx_time_data.rcx_slot_duration << 20;
#endif

        rcx_time_data.rcx_slot_duration = rcx_time_data.rcx_slot_duration / value;
...
void set_sleep_delay(void)
{
    ...
    uint16_t xtal_wait_trim_usec = otp_cs_get_xtal32m_wait_trim();
    ...
    if (arch_clk_is_RCX20())
    {
        ...
        twirq_reset_value = 4 + lld_sleep_us_2_lpcycles_sel_func(RCX_BLE_PWR_UP_TO_SLP_IRQ_USEC + xtal_wait_trim_usec);

        // TWIRQ_SET in LP clock cycles (time to set BLE_WAKEUP_LP IRQ before SLP IRQ assertion)
        twirq_set_value = HW_STARTUP_TIME_IN_RCX_CYCLES + xtal_wait_trim + twirq_reset_value;
    }
    else if (arch_clk_is_XTAL32())
    {
        // TWIRQ_RESET in LP clock cycles (time to reset BLE_WAKEUP_LP IRQ before SLP IRQ assertion)
        // Extra LP clock cycles: 1 due to calibration circuit, 2 to read SLP IRQ status, 1 for margin
        twirq_reset_value = 4 + lld_sleep_us_2_lpcycles_sel_func(XTAL32K_BLE_PWR_UP_TO_SLP_IRQ_USEC + xtal_wait_trim_usec);

        // TWIRQ_SET in LP clock cycles (time to set BLE_WAKEUP_LP IRQ before SLP IRQ assertion)
        twirq_set_value = HW_STARTUP_TIME_IN_XTAL32K_CYCLES + xtal_wait_trim + twirq_reset_value;
    }
#if (!ALL_RAM_RETAINED) && !defined(CFG_REUSE_STARTUP_CODE_RAM)
    ...
    # endif
#endif // USE_XTAL16M_ADAPTIVE_SETTLING
    ...
    if ( arch_clk_is_RCX20() )
    {
        if (rcx_time_data.rcx_period > (RCX_PERIOD_MAX << 10) )
            ASSERT_ERROR(0);
...
#if (USE_XTAL32M_DYN_FREQ_TRIMMING)
static uint8_t xtal32m_calc_trim(uint16_t adc_val)
{
    int32_t result;
    int16_t adc_val_i;
    int16_t i;
    ...
    uint16_t adc_val_rt = otp_cs_get_adc_25_cal();
    if (adc_val_rt == 0)
    {
        // OTP calibration value is not present - use typical value instead
        adc_val_rt = DEFAULT_TEMPERATURE_CAL_VAL;
    }
    adc_val_i = ((adc_val - adc_val_rt) >> 6);

    // the x-axis is a list which starts with adc_min and increments with 1<<trim_delta
    // find the index in the list
    i = (adc_val_i - adc_min) >> trim_delta;
    if (i < 0)
    {
        i = 0;
    }

    // interpolate between (or beyond) index i and index i+1, therefore we should never use the last index:
    if (i > ((sizeof(trimfit_dt) / 2) - 2))
    {
        i = (sizeof(trimfit_dt) / 2) - 2;
    }

    // Low x-value from which we interpolate
    adc_low_val = adc_min + i * (1 << trim_delta);

    // interpolate:
    result = trimfit_dt[i + 1] - trimfit_dt[i];         // delta-y
    result *= (adc_val_i - adc_low_val);                // delta-x
    result = result >> trim_delta;                      // divide by step-size
    result += trimfit_dt[i];                            // add low-side y-value
    result = result >> 6;                               // divide by 64 (trimfit_dt was multiplied by 64, to get sensible integers)
    result += otp_cs_get_xtal32m_trim_value();          // add room-temp trimvalue
...
#endif // USE_XTAL32M_DYN_FREQ_TRIMMING
...
void conditionally_run_radio_cals(void)
{
...
#endif
    {
...
#endif
        {
            ...
            if (rf_cal_last_temp == 127)
            {
                rf_cal_last_temp = current_temp;
            }
            else
            {
                int8_t calibration_temp_diff = current_temp - rf_cal_last_temp;

                if (calibration_temp_diff < 0)
                {
                    calibration_temp_diff = -calibration_temp_diff;
                }

                if (calibration_temp_diff >= CFG_CALIBRATION_TEMP_DIFF)
                {
                    // Calibrate the RF if the temperature difference is greater than
                    // (or equal to) CFG_CALIBRATION_TEMP_DIFF degrees Celsius
                    rf_cal_count++;
                    rf_recalibration();
                    rf_cal_last_temp = current_temp;
                }
...
#if (USE_XTAL32M_DYN_FREQ_TRIMMING)
            if (xtal32m_trim_last_temp == 127)
            {
                xtal32m_trim_last_temp = current_temp;
            }
            else
            {
                int8_t xtal32m_trim_temp_diff = current_temp - xtal32m_trim_last_temp;

                if (xtal32m_trim_temp_diff < 0)
                {
                    xtal32m_trim_temp_diff = -xtal32m_trim_temp_diff;
                }

                if (xtal32m_trim_temp_diff >= CFG_XTAL32M_TRIM_TEMP_DIFF)
                {
                    clk_freq_trim_reg_value = xtal32m_calc_trim(adc_raw_val);
                    // Apply Trim value
...
uint32_t lld_sleep_us_2_lpcycles_rcx_func(uint32_t us)
{
    ...
    lpcycles = (us * rcx_time_data.rcx_freq + 500000) / 1000000;
...
uint32_t lld_sleep_lpcycles_2_us_rcx_func(uint32_t lpcycles)
{
    ...
    if (lpcycles >= 0x8000) {
        ...
        uint64_t res = (uint64_t)lpcycles * rcx_time_data.rcx_period;
        us = res >> 10;
    }
    else {
        us = (lpcycles * rcx_time_data.rcx_period) >> 10;
    }
...
{
    return check_sys_startup_period_ROM(&sys_startup_flag, startup_sleep_delay);
}
#else
uint8_t check_sys_startup_period(void)
{
...
void arch_wkupct_tweak_deb_time(bool tweak)
{
    if (arch_clk_is_RCX20())
    {
        if (tweak)
        {
            ...
            uint32_t temp = (rcx_time_data.rcx_freq * arch_wkupct_deb_time) / 32768;
...
void arch_set_pxact_gpio(GPIO_PORT port, GPIO_PIN pin)
{
    ...
    SetWord16(mode_reg, (uint16_t)PID_GPIO | (uint16_t)OUTPUT);
    SetWord16(set_data_reg, 1 << pin);
    arch_asm_delay_us(47);    // wait for 47 us
...
{
#if (DEVELOPMENT_DEBUG)
    SetWord16(P13_MODE_REG, (uint16_t)PID_GPIO|(uint16_t)OUTPUT);
...
{
    clk_freq_trim_reg_value = trim_value;

#if (defined (__DA14531_01__) || defined (__DA14535__)) && !defined (__EXCLUDE_ROM_ARCH_SYSTEM__)
    set_xtal32m_trim_value_ROM(trim_value);
#else
    ...
    arch_asm_delay_us(1000);                                        // wait for 1 ms
...
#if defined (__DA14531__)
void xtal32m_configuration(void)
{
  #if defined (__DA14535__)
    SetBits16(XTAL32M_CTRL0_REG, CORE_AMPL_REG_NULLBIAS, 1);
  #else
 ...
 ****************************************************************************************
 */
__STATIC_INLINE void xtal32m_calibration_check(void)
{
    // Apply the XTAL32M trim value
    set_xtal32m_trim_value(otp_cs_get_xtal32m_trim_value());
}
#else
/**
 ****************************************************************************************
 ...
 ****************************************************************************************
 */
__STATIC_INLINE void xtal16m_calibration_check(void)
{
    // Get the trim value stored from the booter
    uint16_t trimvalue = GetWord16(CLK_FREQ_TRIM_REG);
#if (DEFAULT_XTAL16M_TRIM_VALUE_USED)
    // Apply the default XTAL16M trim value, if a trim value has not been programmed in OTP.
    // The booter will not apply any trim value to CLK_FREQ_TRIM_REG if the respective OTP
    // entry contains a zero value (a blank OTP contains zeros in DA14585/586).
    // The reset value of CLK_FREQ_TRIM_REG is zero as well.
    if (trimvalue == 0)
    {
        trimvalue = DEFAULT_XTAL16M_TRIM_VALUE;
    }
#endif
    set_xtal16m_trim_value(trimvalue);
}
#endif

void arch_system_init_vdd_clamp_for_rcx(void)
{
#if defined (__DA14535__)
    // Enable LDO_CORE_RET
    SetBits16(POWER_CTRL_REG, LDO_CORE_RET_ENABLE, 1);
    
    arch_asm_delay_us(300);
    SetBits16(POWER_LEVEL_REG, FORCE_RCX_LDO_CORE_RET, 0x1);
...
SYS_INIT_PRE_RUN_ONCE_CODE static void system_init_pre(void)
#else
...
#if defined (__DA14531__)
    // Settings extracted from `set_recommended_settings()`
...
#if defined (__DA14531__) && (USE_TRNG)
    otp_hdr_timestamp = GetWord32(OTP_HDR_TIMESTAMP_ADDR);
#endif
...
SYS_INIT_INTERIM_RUN_ONCE_CODE static void system_init_interim(void)
{
    system_init_pre();
#endif
    ...
    // Create seed with TRNG mechanism and feed it to random number generator function
#if (USE_TRNG)
...
#if !defined (__NON_BLE_EXAMPLE__)
// Use ROM 'gap_cfg_user_var_struct symbol' only for DA14531, DA14531-01, DA14535 and ROM SDK combination
...
const struct gap_cfg_user_struct gap_cfg_user_var_struct =
{
    ...
    .GAP_DEV_NAME_VAR="DA14531-01",
#elif defined (__DA14535__)
    .GAP_DEV_NAME_VAR="DA14535",
#elif defined (__DA14531__)
