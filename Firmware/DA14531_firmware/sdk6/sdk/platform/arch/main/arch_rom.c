 ...
 ****************************************************************************************
 */
SYS_INIT_INTERIM_RUN_ONCE_CODE void arch_rom_init(void);
...
void arch_rom_init(void)
{
...
#if defined (__DA14531__) && (USE_TRNG) && !defined(CFG_REUSE_STARTUP_CODE_RAM)
    ...
    /* Do not erase the startup code yet */
    memset(temp_ptr, 0, CO_ALIGN4_HI(EXCH_MEM_TX_CNTL_BUFFER_BASE_ADDR - __SCT_BLE_BASE));
#else
...
#else
    struct le_features my_llm_local_le_feats = {{ BLE_FEATURES_BYTE0, 0, 0, 0, 0, 0, 0, 0 }};
    memcpy(&custom_llm_local_le_feats, &my_llm_local_le_feats, FEATS_LEN);
#endif
...
{
    uint32_t *temp_ptr = (uint32_t *) CO_ALIGN4_HI(EXCH_MEM_TX_CNTL_BUFFER_BASE_ADDR);
#if defined(__IS_SDK6_COMPILER_GCC__) && !defined(__clang__)
    #pragma GCC diagnostic push
    #pragma GCC diagnostic ignored "-Wstringop-overflow"
    #pragma GCC diagnostic ignored "-Warray-bounds"
#endif
    memset(temp_ptr, 0, CO_ALIGN4_HI(__SCT_EM_BLE_END - EXCH_MEM_TX_CNTL_BUFFER_BASE_ADDR + __SCT_BLE_BASE));
#if defined(__IS_SDK6_COMPILER_GCC__) && !defined(__clang__)
    #pragma GCC diagnostic pop
#endif
}
...
__SECTION("system_DA14xxx") void fill_dbheap_random_data(void)
#else
void fill_dbheap_random_data(void)
#endif
{
    if (trng_state_val != TRNG_MAGIC_NUMBER)
    {
