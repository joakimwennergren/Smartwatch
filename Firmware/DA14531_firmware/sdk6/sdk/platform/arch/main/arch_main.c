...
#if !defined (__DA14531__)
#if !defined (CODELESS_585) // When codeless is defined the definitions are in arch_ram.h
...
int main(void)
{
    ...
    init_retention_mode();
#else
    arch_ram_set_retention_mode(ALL_RAM_BLOCKS);
#endif
    ...
    extern uint8_t check_for_nmi_or_hardfault(void);
    check_for_nmi_or_hardfault();
#endif
    ...
    while(1)
    {
        ...
        if (((!BLE_APP_PRESENT) && (check_gtl_state())) || (BLE_APP_PRESENT))
        {
            ...
            else if (sleep_mode == mode_idle)
            {
                ...
                {
#if (USE_WDOG)
                    // If the watchdog is enabled and there is no interrupt to wake the
                    // system up a watchdog timeout will occur.Therefore the watchdog is
                    // frozen before the __WFI and restored afterwards
