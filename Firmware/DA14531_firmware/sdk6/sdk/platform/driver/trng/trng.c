...
#endif
    arch_asm_delay_us(100); // Wait 100 us to finish all DCF signals
}
...
//    SetWord16(RF_SPARE1_REG,          0x0001);   // Set the IFF in REAL transfer function, to remove I-Q correlation. But it affects the DC offsets!
    ...
    arch_asm_delay_us(380);             // Wait for the DCFs to finish
}
...
#if defined (__DA14531__)
SYS_INIT_INTERIM_RUN_ONCE_CODE void init_rand_seed_from_trng(void);
volatile uint32_t trng_state_val            __SECTION_ZERO("trng_state"); // uninitialized variable placed always in last RAM block
...
{
#if defined (__DA14531__)
    // Decide whether or not to trigger the TRNG mechanism for the seed number
    // generation. The TRNG mechanism uses an uninitialized memory space in
    // order to calculate the seed number for the random number generator
    // functions (stdlib/rand.c or chacha20.c).
    //
    // The following check relies on the uninitialized value of the
    // `trng_state_val` variable. This variable will have a random value after
    // a power cycle or if the last RAM block was turned off during sleep or
    // hibernation.
    //
    // Note: The size of the variable may be increased in order to make the
    //       check more robust.

    if (trng_state_val != TRNG_MAGIC_NUMBER)
    {
        uint8_t status = IID_SUCCESS;
        uint8_t *buffer = (uint8_t *) DB_HEAP_BASE;
        *((unsigned int *) BLE_EM_BASE_REG) = _ble_base;    // Required for AES
        ...
        // TRNG has run
        trng_state_val = TRNG_MAGIC_NUMBER;
