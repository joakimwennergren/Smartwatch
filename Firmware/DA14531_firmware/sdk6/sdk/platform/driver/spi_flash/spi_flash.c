...
int8_t spi_flash_auto_detect(uint8_t *dev_id)
{
    ...
    // At this point a flash exists but has not been recognized
    // Use a default spi flash size for CodeLess
    spi_flash_cfg_t spi_flash_default_cfg = 
    {
        .jedec_id = 0, 
        .dev_index = 0,
#if defined(SPI_FLASH_DEFAULT_SIZE)        
        .chip_size = SPI_FLASH_DEFAULT_SIZE,
#else
        #error SPI_FLASH_DEFAULT_PAGE_SIZE macro is undefined.Please provide a default value
#endif
    };
    spi_flash_configure_env(&spi_flash_default_cfg);
...
int8_t spi_flash_configure_memory_protection(uint8_t data)
{
    ...
    if (spi_flash_cfg_env.dev_index == AT25xy021A_DEV_INDEX) {
        if (data == SPI_FLASH_MEM_PROT_NONE)
        {
            apply_value = ((status_reg & AT25xy021A_MEM_PROT_STATUS) != 0);
        }
    } else {
        apply_value = ((status_reg & SPI_FLASH_MEM_PROT_MASK) != (data & SPI_FLASH_MEM_PROT_MASK));
    }
    if (apply_value)
    {
        status = spi_flash_write_status_reg((status_reg & (~SPI_FLASH_MEM_PROT_MASK))
...
{
    arch_asm_delay_us(100);    // delay for 100us
}
...
bool spi_flash_enable_power_pin(void)
{
    ...
    if (!is_pin_already_enabled)
    {
        ...
        arch_asm_delay_us(1000);    // delay for 1ms
