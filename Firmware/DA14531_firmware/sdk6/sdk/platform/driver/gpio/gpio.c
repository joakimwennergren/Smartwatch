...
#if defined(GPIO_OPT)
int8_t GPIO[NO_OF_MAX_PINS_PER_PORT]                        __SECTION_ZERO("retention_mem_area0");
static const uint16_t p_mask = 0xFFF;
volatile uint16_t GPIO_status                               __SECTION_ZERO("retention_mem_area0");
#else
...
#if (defined (__DA14531_01__) || defined (__DA14535__)) && !defined (__EXCLUDE_ROM_GPIO__)
 ...
 ****************************************************************************************
 */
SYS_INIT_PRE_RUN_ONCE_CODE void GPIO_init(void);
...
void GPIO_init(void)
{
    ...
    if (arch_clk_is_XTAL32() == true)
    {
        ...
        #if defined(GPIO_OPT)
        ASSERT_WARNING((GPIO[3] == 0) && (GPIO[4] == 0));
        #else
    ...
    // NOTE: For DA14533 device, pins P06, P09 and P011 are not available
    ASSERT_WARNING((GPIO[0][6] == 0) && (GPIO[0][9] == 0) && (GPIO[0][11] == 0));
#endif
    ...
    for (j = 0; j < NO_OF_MAX_PINS_PER_PORT; j++) 
    {
        ...
        if ( !(p_mask & bitmask) ) // port pin does not exist! continue to next port...
            break;
...
#endif
#if defined(__DA14531__) && defined(GPIO_OPT)
    const int mode_reg = P00_MODE_REG + (pin << 1);
#else
...
#else
#if defined (__DA14531__) && defined(GPIO_OPT)
void GPIO_ConfigurePinPort0(GPIO_PIN pin, GPIO_PUPD mode, GPIO_FUNCTION function,
                        const bool high)
{
#if DEVELOPMENT_DEBUG
    #ifndef GPIO_DRV_PIN_ALLOC_MON_DISABLED
...
#endif
    if (high)
        GPIO_SetActivePort0(pin);
    else
        GPIO_SetInactivePort0(pin);
...
#endif
#if defined(__DA14531__) && defined(GPIO_OPT)
    const int set_data_reg = P0_SET_DATA_REG;
#else
    const int data_reg = GPIO_BASE + (port << 5);
...
#endif
#if defined(__DA14531__) && defined(GPIO_OPT)
    const int reset_data_reg = P0_RESET_DATA_REG;
#else
    const int data_reg = GPIO_BASE + (port << 5);
...
#endif
#if defined(__DA14531__) && defined(GPIO_OPT)
    const int data_reg = P0_DATA_REG;
#else
    const int data_reg = GPIO_BASE + (port << 5);
#endif 
...
#else
#if defined (__DA14531__) && defined(GPIO_OPT)
void GPIO_EnableIRQPort0(GPIO_PIN pin, IRQn_Type irq, bool low_input,
                     bool release_wait, uint8_t debounce_ms)
#else
void GPIO_EnableIRQ(GPIO_PORT port, GPIO_PIN pin, IRQn_Type irq, bool low_input,
                     bool release_wait, uint8_t debounce_ms)
...
#if defined (__DA14531__) && defined(GPIO_OPT)
    GPIOSetBits16(GPIO_IRQ0_IN_SEL_REG + 2*(irq-GPIO0_IRQn), KBRD_IRQn_SEL, 1 + pin);
#else
