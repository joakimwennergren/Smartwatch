...
{
    uint8_t reg = GetWord8(CLK_AMBA_REG);
    reg |= (PCLK_DIV | HCLK_DIV);
    SetWord8(CLK_AMBA_REG, reg);
}
...
void syscntl_por_vbat_high_cfg(por_vbat_mask_t mask)
{
    ...
    SetBits16(POWER_AON_CTRL_REG, POR_VBAT_HIGH_RST_MASK, mask);
}
...
void syscntl_por_vbat_low_cfg(por_vbat_mask_t mask)
{
    ...
    SetBits16(POWER_AON_CTRL_REG, POR_VBAT_LOW_RST_MASK, mask);
}
