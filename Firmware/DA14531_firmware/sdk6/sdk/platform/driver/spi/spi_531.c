 ...
 * @file spi_531.c
 *
 * @brief Implementation of the SPI Low Level Driver for DA14531/DA14535 device.
 *
...
typedef struct
{
#ifndef CFG_SPI_STATIC_CONFIGURATION
...
#if defined (__DA14531_01__) && !defined (__EXCLUDE_ROM_SPI_531__)
extern int8_t spi_initialize_ROM(const spi_cfg_t *spi_cfg);
extern int8_t spi_master_receive(void *rx_buf, uint16_t length, SPI_OP_CFG op);
extern int8_t spi_slave_receive(void *rx_buf, uint16_t length, SPI_OP_CFG op);
extern int8_t spi_slave_transfer(const void *tx_buf, void *rx_buf, uint16_t length, SPI_OP_CFG op);
extern SPI_WSZ_MODE_CFG spi_get_wsz(void);
extern uint8_t spi_get_incr(void);
#endif

// Required DA14535 ROM symbols
#if defined (__DA14535__) && !defined (__EXCLUDE_ROM_SPI_531__)
extern int8_t spi_initialize_ROM(const spi_cfg_t *spi_cfg);
extern int8_t spi_master_send(const void *tx_buf, uint16_t length, SPI_OP_CFG op);
extern int8_t spi_master_receive(void *rx_buf, uint16_t length, SPI_OP_CFG op);
extern int8_t spi_slave_send(const void *tx_buf, uint16_t length, SPI_OP_CFG op);
extern int8_t spi_slave_receive(void *rx_buf, uint16_t length, SPI_OP_CFG op);
extern int8_t spi_slave_transfer(const void *tx_buf, void *rx_buf, uint16_t length, SPI_OP_CFG op);
extern SPI_WSZ_MODE_CFG spi_get_wsz(void);
extern uint8_t spi_get_incr(void);
...
#if (!defined (__DA14531_01__) && !defined (__DA14535__)) || defined (__EXCLUDE_ROM_SPI_531__)
#ifdef CFG_SPI_STATIC_CONFIGURATION

#define SPI_WSZ_TO_BITS(x) (x == SPI_MODE_8BIT ? 8 : (x == SPI_MODE_16BIT ? 16 : 32))
...
static const spi_cfg_t spi_static_cfg = {
    .spi_ms = SPI_MS_MODE,
    .spi_cp = SPI_CP_MODE,
...
#define MAX_LEADING_ZEROS             ((uint8_t) (__CLZ(7)))    // 7 is the lowest SPI_WORD_LENGTH

static uint8_t spi_get_incr(void)
{
    // Get the SPI_WORD_LENGTH (7 for 8-bit, 15 for 16-bit and 31 for 32-bit)
    uint8_t res = GetBits16(&spi->SPI_CONFIG_REGF, SPI_WORD_LENGTH);
    return((res + 1) / 8);
}

static SPI_WSZ_MODE_CFG spi_get_wsz(void)
{
    // Get the SPI_WORD_LENGTH (7 for 8-bit, 15 for 16-bit and 31 for 32-bit)
    uint8_t res = GetBits16(&spi->SPI_CONFIG_REGF, SPI_WORD_LENGTH);

    // Get the negative representation of SPI_WSZ_MODE_CFG
    int32_t n = (uint8_t) (__CLZ(res)) - MAX_LEADING_ZEROS;

    // Get the absolute value of the signed representation of SPI_WSZ_MODE_CFG
    int32_t const mask = n >> ((sizeof(uint32_t) * CHAR_BIT) - 1);
    return (SPI_WSZ_MODE_CFG) ((n + mask) ^ mask);
}
#endif // __EXCLUDE_ROM_SPI_531__
...
{
#ifdef CFG_SPI_STATIC_CONFIGURATION
    if (spi_static_cfg.spi_ms == SPI_MS_MODE_MASTER)
#else
...
{
    SPI_WSZ_MODE_CFG wsz = spi_get_wsz();

    // Update DMA bus data width
    dma_set_bw(spi_env.spi_dma_tx_channel, wsz == SPI_MODE_8BIT ? DMA_BW_BYTE : DMA_BW_HALFWORD);
...
static void spi_dma_receive(const uint32_t *rx_buf, uint16_t length, dma_cb_t cb, DMA_IRQ_CFG irq_en)
{
    ...
    dma_set_bw(spi_env.spi_dma_rx_channel, wsz == SPI_MODE_8BIT ? DMA_BW_BYTE : DMA_BW_HALFWORD);
...
{
    while(dma_get_ctrl_reg(spi_env.spi_dma_rx_channel) & DMA_ON);
}
...
static int8_t spi_slave_receive(void *rx_buf, uint16_t length, SPI_OP_CFG op)
{
    ...
    if (op == SPI_OP_BLOCKING || wsz == SPI_MODE_32BIT)
    {
        // Enable SPI
        spi_ctrl_reg_spi_en_setf(SPI_BIT_EN);
...
static int8_t spi_master_send(const void *tx_buf, uint16_t length, SPI_OP_CFG op)
{
    ...
    if ((op == SPI_OP_DMA) && (wsz != SPI_MODE_32BIT))
    {
...
void spi_set_speed(SPI_SPEED_MODE_CFG spi_speed)
{
    ...
    {
        spi_clock_reg_clk_div_setf((SPI_MASTER_CLK_FREQ_32 / ( spi_speed << 1))-1);
    }
}
#endif // __EXCLUDE_ROM_SPI_531__
...
int8_t spi_initialize(const spi_cfg_t *spi_cfg)
{
    ...
    return spi_initialize_ROM(spi_cfg);
}
#else
int8_t spi_initialize(const spi_cfg_t *spi_cfg)
{
...
#ifdef CFG_SPI_STATIC_CONFIGURATION
    ASSERT_WARNING(spi_cfg == NULL);

    // Set SPI Word length
    spi_set_bitmode(spi_static_cfg.spi_wsz);
    
    // Set SPI Mode (CPOL, CPHA)
    spi_set_cp_mode(spi_static_cfg.spi_cp);

    // Set SPI Master/Slave mode
    spi_config_reg_ms_mode_setf(spi_static_cfg.spi_ms);
    ...
    {
        // Set SPI master clock speed
        spi_set_speed(spi_static_cfg.spi_speed);

        // Set SPI clock edge capture data
        spi_ctrl_reg_capture_next_edge_setf(spi_static_cfg.spi_capture);
    }
...
void spi_cs_low(void)
{
    spi_ctrl_reg_spi_fifo_reset_setf(SPI_BIT_DIS);
#ifdef CFG_SPI_STATIC_CONFIGURATION
    spi_cs_mode_setf(spi_static_cfg.spi_cs);
#else
...
int8_t spi_receive(void *data, uint16_t num, SPI_OP_CFG op)
{
    ...
    if (spi_env.spi_ms == SPI_MS_MODE_SLAVE)
    {
        uint8_t status = spi_slave_receive(data, num, op);

        #if (defined (__DA14531_01__) || defined (__DA14535__)) && !defined (__EXCLUDE_ROM_SPI_531__)
            #if defined (CFG_SPI_DMA_SUPPORT)
            {
                // Enable Rx DMA
                spi_dma_receive((uint32_t *) data, num, spi_dma_receive_cb, DMA_IRQ_STATE_ENABLED);
                // Enable SPI
                spi_ctrl_reg_spi_en_setf(SPI_BIT_EN);
                status = SPI_STATUS_ERR_OK;
            }
            #endif
        #endif
...
int8_t spi_transfer(const void *data_out, void *data_in, uint16_t num, SPI_OP_CFG op)
{
    ...
    if (spi_env.spi_ms == SPI_MS_MODE_SLAVE)
    {
        uint8_t status = spi_slave_transfer(data_out, data_in, num, op);

        #if (defined (__DA14531_01__) || defined (__DA14535__)) && !defined (__EXCLUDE_ROM_SPI_531__)
            #if defined (CFG_SPI_DMA_SUPPORT)
                // Enable Rx DMA
                spi_dma_receive((uint32_t *) data_in, num, spi_dma_transfer_cb, DMA_IRQ_STATE_ENABLED);
                // Enable Tx DMA
                data_out += (1 << spi_get_wsz());
                spi_dma_send((uint32_t *) data_out, num - 1, NULL, DMA_IRQ_STATE_DISABLED);
            #endif
        #endif
