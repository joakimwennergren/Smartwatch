 ...
 * @file uart.c
 *
 * @brief UART Low Level Driver for DA14585/586/531/535 devices.
 *
...
#define UART_ONE_CHAR_DELAY         (350) // 350 corresponds to 265us delay
...
static uart_env_t uartn_env[1] __SECTION_ZERO("retention_mem_area0");
#else
/// UART environment, retained in retention memory area.
...
#if (defined (__DA14531_01__) || defined (__DA14535__)) && !defined (__EXCLUDE_ROM_UART__)
extern bool uart_disable_flow_control_ROM(uart_t *uart_id, uint32_t retries, uint32_t one_char_delay);
#endif
...
static void uart_tx_isr(uart_t *uart_id)
{
    ...
    if (uart_env->tx_index < uart_env->tx_total_length)
    {
...
#if defined (CFG_UART1_SDK) || !defined(CFG_UART2_DISABLE)
/**
 ****************************************************************************************
...
#if !defined(__EXCLUDE_UART1_HANDLER__)
/**
 ****************************************************************************************
...
#endif //__EXCLUDE_UART1_HANDLER__
#endif
...
{
    return uart_disable_flow_control_ROM(uart_id, retries, UART_ONE_CHAR_DELAY);
}
#else
bool uart_disable_flow_control(uart_t *uart_id, uint32_t retries)
{
    ...
    // same time
    arch_asm_delay_us(UART_ONE_CHAR_DELAY - 85);
