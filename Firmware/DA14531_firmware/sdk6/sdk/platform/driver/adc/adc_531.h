...
typedef struct
{
   ...
   union {
        struct {
            // GP_ADC_CTRL_REG
            uint8_t             reserved:2;             // bit0:1: GP_ADC_EN, GP_ADC_START
            /// Continuous mode off/on
            bool                continuous:1;           // bit2
            uint8_t             reserved2:3;            // bit3:5: GP_ADC_DMA_EN, GP_ADC_INT, GP_ADC_MINT
            /// Input channel mode
            adc_input_mode_t    input_mode:1;           // bit6
            bool                mute:1;                 // bit7
            bool                sign:1;                 // bit8
            /// Chopper mode off/on
            bool                chopping:1;             // bit9
            uint8_t             reserved3:1;            // bit10 GP_ADC_LDO_HOLD
            adc_input_sh_gain_t input_shifter_gain:1;   // bit11
            bool                temp_sensor_enable:1;   // bit12
            uint8_t reserved4:3;                        // bit13:15
            // GP_ADC_CTRL2_REG
            /// Attenuator
            adc_input_attn_t    input_attenuator:2;     // bit0:1
            /// 20-uA constant load current at the ADC LDO output
            bool                ldo_const_current:1;    // bit2
            bool                input_shifter:1;        // bit3
            adc_input_sh_cm_t   input_shifter_cm:2;     // bit4:5
            /// Oversampling mode
            uint16_t            oversampling:3;         // bit6:8 Must be uint16_t to cross the byte boundary
            /// Sample time
            uint8_t             smpl_time_mult:4;       // bits9:12
            uint8_t             store_delay:3;          // bits13:15
            // GP_ADC_CTRL2_REG
            // The delay taking place after LDO has been enabled and before ADC
            // gets enabled. Delay is equal to (delay_factor) * 4 * (ADC_CLK period).
            uint8_t             delay_factor;           // bits0:7
            /// Interval between conversions when in continuous mode
            uint8_t             interval_mult;          // bits8:15
            // GP_ADC_SEL_REG
            /// Negative input channel (diff mode only)
            adc_input_diff_t    input_n:4;              // bits0:3
            adc_input_se_t      input:4;                // bits4:7
        } b;
        struct {
            uint16_t ctrl_reg;
            uint16_t ctrl2_reg;
    ...
    /// Input channel; possible values: @c adc_input_se_t for single ended mode
    /// or  ( @c adc_input_se_t | @c adc_input_diff_t << 4) for differential mode, e.g.
    /// (ADC_INPUT_SE_P0_1 | (ADC_INPUT_DIFF_P0_2 << 4)) initializes the P01 as positive
    /// channel of ADC and P0_2 as negative channel of ADC in differential mode.
    uint8_t             input;
 ...
 * @note Code shall be built using compiler version 6 with size optimization
 ...
 * @note Not applicable to DA14535
 ****************************************************************************************
 */
#if defined(ADC_OPT)
__STATIC_FORCEINLINE void adc_input_shift_enable(void)
{
 ...
 */
#if defined(ADC_OPT)
__STATIC_FORCEINLINE void adc_input_shift_disable(void)
{
 ...
 * @param enable true to enable continuous mode, false to disabled it
 ...
 */
 #if defined(ADC_OPT)
__STATIC_FORCEINLINE void adc_continuous_set(bool enable)
{
    SetBits16(GP_ADC_CTRL_REG, GP_ADC_CONT, enable ? 1 : 0);
}
#endif 
 ...
 * @note In the case of differential mode returns the positive
 * input channel.
...
/**
 ****************************************************************************************
 * @brief Returns the negative input channel in differential mode.
...
/**
 ****************************************************************************************
 * @brief Sets chopper mode.
 * @param enable true to enable chopper mode, false to disabled it
 * @return void
 ****************************************************************************************
 */
 #if defined(ADC_OPT)
__STATIC_FORCEINLINE void adc_chopper_set(bool enable)
{
    SetBits16(GP_ADC_CTRL_REG, GP_ADC_CHOP, enable? 1 : 0);
}
#endif
/**
 ****************************************************************************************
...
/**
 ****************************************************************************************
 * @brief Initializes and enables ADC for temperature measurement.
 * @return void
 ****************************************************************************************
 */
...
/**
 ****************************************************************************************
 * @brief Sets the die temperature sensor.
 * @param enable true to enable temperature sensor, false to disabled it
 * @return void
 ****************************************************************************************
 */
__STATIC_FORCEINLINE void adc_temp_sensor_set(bool enable)
{
    SetBits16(GP_ADC_CTRL_REG, DIE_TEMP_EN, enable ? 1: 0);
 ...
 */
#if defined(ADC_OPT)
 __STATIC_FORCEINLINE void adc_temp_sensor_enable(void)
{
 ...
 ****************************************************************************************
 */
__STATIC_FORCEINLINE void adc_temp_sensor_disable(void)
{
...
/**
 ****************************************************************************************
 * @brief Store offset calibration values.
 ****************************************************************************************
 */
...
/**
 ****************************************************************************************
 * @brief Restore offset calibration values.
 ****************************************************************************************
 */
...
/**
 ****************************************************************************************
 * @brief Reset offset calibration values.
 ****************************************************************************************
 */
