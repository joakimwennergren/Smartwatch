...
static const adc_config_t adc_temp_sens_cfg =
{
    .adc_cfg.b.temp_sensor_enable   = true,
    .adc_cfg.b.input_mode           = ADC_INPUT_MODE_SINGLE_ENDED,
    .adc_cfg.b.input                = ADC_INPUT_SE_TEMP_SENS,
    .adc_cfg.b.smpl_time_mult       = 15,
    .adc_cfg.b.continuous           = false,
    .adc_cfg.b.interval_mult        = 0,
    .adc_cfg.b.input_attenuator     = ADC_INPUT_ATTN_NO,
    .adc_cfg.b.chopping             = true,
    .adc_cfg.b.oversampling         = 6,
    .adc_cfg.b.ldo_const_current    = true,
};
#endif // ADC_OPT
...
__attribute__((unused)) static void adc_configure(const adc_config_t *cfg)
{
    ...
    {
#if defined(ADC_OPT)
        uint16_t reg = GetWord16(GP_ADC_CTRL_REG);
        reg &= ~(GP_ADC_EN | GP_ADC_START | GP_ADC_DMA_EN | GP_ADC_INT | GP_ADC_MINT | GP_ADC_LDO_HOLD);
        reg |= cfg->adc_cfg.w.ctrl_reg;
        SetWord16(GP_ADC_CTRL_REG, reg);

        SetWord16(GP_ADC_CTRL2_REG, cfg->adc_cfg.w.ctrl2_reg);

        reg = cfg->adc_cfg.w.ctrl3_reg;
        if (cfg->adc_cfg.b.delay_factor == 0) {
            // Delay factor cannot be zero. Load the reset value
            reg |= GP_ADC_CTRL3_REG_RESET & GP_ADC_EN_DEL;
        }
        SetWord16(GP_ADC_CTRL3_REG, reg);

        SetWord8(GP_ADC_SEL_REG, cfg->adc_cfg.w.sel_reg);

        if (cfg->adc_cfg.b.temp_sensor_enable) {
            // Guideline from the Analog IC Team: Wait for 25us to let the temperature
            // sensor settle just after enabling it
            usDelay(25);
        }

        if (cfg->adc_cfg.b.input_shifter) {
...
#if !defined (CFG_ADC_DISABLE_CALIBRATION)
...
#endif
#endif // CFG_ADC_DISABLE_CALIBRATION
...
{
#if defined(ADC_OPT)
    SetBits16(GP_ADC_SEL_REG, GP_ADC_SEL_P | GP_ADC_SEL_P_TST, input);
#else
...
{
#if defined(ADC_OPT)
    SetBits16(GP_ADC_SEL_REG, GP_ADC_SEL_N | GP_ADC_SEL_N_TST, input);
#else
...
#if (!defined (__DA14531_01__) && !defined (__DA14535__)) || defined (__EXCLUDE_ROM_ADC_531__)
#if defined(ADC_OPT)
// Configuration for offset calibration
static const adc_config_t adc_offset_calib_cfg =
{
    .adc_cfg.b.smpl_time_mult       = 1,
    .adc_cfg.b.chopping             = false,
    .adc_cfg.b.oversampling         = 7,
    .adc_cfg.b.ldo_const_current    = true,
    .adc_cfg.b.mute                 = true,
    .adc_cfg.b.sign                 = false,
};
#endif
#endif
...
#else
    return (adc_val >> (6 - ADC_531_MIN(6, adc_get_oversampling())));
#endif
}
#endif // __EXCLUDE_ROM_ADC_531__
