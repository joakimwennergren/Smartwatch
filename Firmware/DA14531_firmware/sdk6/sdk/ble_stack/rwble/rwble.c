 ...
 * Copyright (C) 2017-2024 Modified by Renesas Electronics Corporation and/or its affiliates.
 *
 ****************************************************************************************
 */
...
#include "user_at_commands.h"
#if defined (USE_AT_PWRLVL)
#define __EXCLUDE_ROM_RWBLE__
#endif // USE_AT_PRWLVL
#endif // __DA14535__ 

#if ENABLE_WAKEUP_METRICS
...
#define INDEX_0US           0
#define INDEX_500US         1
#define INDEX_1000US        2
#define INDEX_1250US        3
#define INDEX_1500US        4
#define INDEX_2000US        5
#define INDEX_2500US        6
#define INDEX_3000US        7
#define INDEX_MAX           8

// Holds the statistics of the wake up delay
uint32_t stats_array[INDEX_MAX]         __SECTION_ZERO("retention_mem_area0");
// Holds the max diff in the case of an assertion
int max_diff_lp                         __SECTION_ZERO("retention_mem_area0");
int max_diff_us                         __SECTION_ZERO("retention_mem_area0");
uint32_t wakeup_cnt                     __SECTION_ZERO("retention_mem_area0");
uint32_t wakeup_delayed                 __SECTION_ZERO("retention_mem_area0");

static void measure_stats(int time_lp)
{
    uint32_t temp_diff_us = lld_sleep_lpcycles_2_us_sel_func(time_lp);
    
    if (temp_diff_us >= DELAY_3000US)
        stats_array[INDEX_3000US]++;
    else if (temp_diff_us >= DELAY_2500US)
        stats_array[INDEX_2500US]++;
    else if (temp_diff_us >= DELAY_2000US)
        stats_array[INDEX_2000US]++;
    else if (temp_diff_us >= DELAY_1500US)
        stats_array[INDEX_1500US]++;
    else if (temp_diff_us >= DELAY_1250US)
        stats_array[INDEX_1250US]++;
    else if (temp_diff_us >= DELAY_1000US)
        stats_array[INDEX_1000US]++;
    else if (temp_diff_us >= DELAY_500US)
        stats_array[INDEX_500US]++;
    else
        stats_array[INDEX_0US]++;
    
    if(max_diff_lp < time_lp)
    {
        max_diff_lp = time_lp;
        max_diff_us = temp_diff_us;
    }
}
...
#if (defined (__DA14531_01__) || defined (__DA14535__)) && !defined (__EXCLUDE_ROM_RWBLE__)
typedef void (*ble_isr_t) (void);
...
extern void BLE_WAKEUP_LP_Handler_ROM(uint16_t clk_freq_trim_reg_value, uint16_t xtal_wait_trim);
extern void rwble_isr_ROM(const ble_irq_t *ble_irq);
...
static void update_ble_metrics(uint8_t pkts)
{
    ...
    while(rx_cnt--)
    {
        ...
        {
            uint16_t rx_rssi = (uint16_t) llc_util_rxrssi_getf(rxdesc);

            if (!metrics.rx_pkt)
            {
                metrics.rx_rssi = rx_rssi;
                metrics.rx_pkt++;
            }
            else
            {
                ...
                metrics.rx_pkt++;
                metrics.rx_rssi = (uint16_t)((tmp_sum + rx_rssi)/(metrics.rx_pkt));
            }
        }
...
#if ENABLE_WAKEUP_METRICS
        wakeup_delayed++;
        measure_stats((int)(slp_period - sleep_lp_cycles));
#endif // ENABLE_WAKEUP_METRICS
    }
#endif
#endif
...
{
    /* 
    For DA1458x devices in high temperature configuration care must be taken when additional 
    code is added within the BLE_WAKEUP_LP_HANDLER_ENTER function since the Bluetooth(R) timings may 
    be affected.Please also check CFG_ENABLE_WAKEUP_METRICS macro and lld_sleep_compensate_func.
    */
    ...
    if (GetBits16(CLK_CTRL_REG, SYS_CLK_SEL) != 0)
    {
        ...
        if (GetBits16(SYS_STAT_REG, PER_IS_DOWN))
        {
              /* 
            For DA1458x devices in high temperature configuration care must be taken when additional 
            code is added within the periph_init function since the Bluetooth(R) timings may be affected.
            Please also check CFG_ENABLE_WAKEUP_METRICS macro and lld_sleep_compensate_func.
            */
            periph_init();
        }
...
#if (defined (__DA14531_01__) || defined (__DA14535__)) && !defined (__EXCLUDE_ROM_RWBLE__)
    BLE_WAKEUP_LP_Handler_ROM(clk_freq_trim_reg_value, xtal_wait_trim);
#else
#if defined (__DA14531__)
    // Lower clocks
    syscntl_use_lowest_amba_clocks();
    ...
    /* 
    For DA1458x devices in high temperature configuration care must be taken when additional 
    code is added within the BLE_WAKEUP_LP_HANDLER_EXIT function since the Bluetooth(R) timings may 
    be affected.Please also check CFG_ENABLE_WAKEUP_METRICS macro and lld_sleep_compensate_func.
    */
...
/* 
    For DA1458x devices in high temperature configuration care must be taken when additional 
    code is added within this function including the POWER_UP_ENTER() and POWER_UP_EXIT() functions 
    since the Bluetooth(R) timings may be affected.Please also check CFG_ENABLE_WAKEUP_METRICS macro 
    and lld_sleep_compensate_func.
*/
void power_up(void)
{
    POWER_UP_ENTER();
...
__attribute__((unused)) __STATIC_INLINE void dlg_sw_isr(void)
{
    ea_sw_isr();
}

__attribute__((unused)) __STATIC_INLINE void dlg_grosstgtim_isr(void)
{
    lld_evt_timer_isr();
}

__attribute__((unused)) __STATIC_INLINE void dlg_crypt_isr(void)
{
    lld_crypt_isr();
}
...
#if (defined (__DA14531_01__) || defined (__DA14535__)) && !defined (__EXCLUDE_ROM_RWBLE__)
const ble_irq_t ble_irq[] =
{
    // The order in the table defines the priority of the BLE controller interrupt service routine execution
    {BLE_FINETGTIMINTSTAT_BIT,  dlg_finetgtim_isr},
    {BLE_RXINTSTAT_BIT,         dlg_rx_isr},
    {BLE_EVENTINTSTAT_BIT,      dlg_event_isr},
    {BLE_EVENTAPFAINTSTAT_BIT,  dlg_eventapfa_isr},
    {BLE_SWINTSTAT_BIT,         ea_sw_isr},
    {BLE_GROSSTGTIMINTSTAT_BIT, lld_evt_timer_isr},
    {BLE_CRYPTINTSTAT_BIT,      lld_crypt_isr},
    {BLE_SLPINTSTAT_BIT,        dlg_slp_isr},
    {BLE_CSCNTINTSTAT_BIT,      dlg_cscnt_isr},
    {BLE_ERRORINTSTAT_BIT,      dlg_error_isr},
    {0,                         NULL}
};
#endif // __EXCLUDE_ROM_RWBLE__
...
__BLEIRQ void rwble_isr(void)
{
    ...
    while (1)
    {
        ...
        if (irq_stat & BLE_EVENTINTSTAT_BIT)
        {
            ...
            dlg_event_isr();
#if defined(__DA14531__) && defined(USE_AT_PWRLVL)
            set_power_level();
#endif
...
uint8_t rf_ldo_calibrated_value     __SECTION_ZERO("retention_mem_area0");
#endif
#endif
...
//TXEN RISING EDGE
    if (rf_diagirq_stat & RF_DIAG_IRQ_MODE_TX_RISING) //TXEN rising edge
    {
        ble_tx_evt_handler();
    }

//TXEN FALLING EDGE
    if (rf_diagirq_stat & RF_DIAG_IRQ_MODE_TX_FALLING)  //TXEN falling edge
    {
    }

//RXEN RISING EDGE
    if (rf_diagirq_stat & RF_DIAG_IRQ_MODE_RX_RISING) //RXEN rising edge
    {
#if (USE_RF_LDO_CALIBRATION)
        SetBits16(RF_LDO_CTRL_REG, LDO_RADIO_LEVEL, rf_ldo_calibrated_value);
#endif
        ble_rx_evt_handler();
    }

//RXEN FALLING EDGE
    if (rf_diagirq_stat & RF_DIAG_IRQ_MODE_RX_FALLING) //RXEN falling edge
    {
#if (USE_RF_LDO_CALIBRATION)
        extern uint8_t rf_ldo_init_value;
        SetBits16(RF_LDO_CTRL_REG, LDO_RADIO_LEVEL, rf_ldo_init_value); // recover the value back
#endif
    }
}
#else
...
void ce_len_set(void)
{
    ...
    if ((codeless_env.bt_role == GAP_ROLE_CENTRAL) || (codeless_env.in_binary_mode != true))
#else
    if (codeless_env.in_binary_mode != true)
#endif // __DA14531_01__
    {
        ble_maxevtime_setf(0, 0);
    }
    else
    {
        ble_maxevtime_setf(0, PERIPHERAL_CE_LEN_LIMIT);
    }
}
#endif //CFG_PERIPHERAL_CE_LEN_LIMITING_ENABLED
