...
#ifdef CODELESS_585
    /// Inform app that a new command message is received. Either from uart or from command sequencer.
...
/// Parameters of the @ref USER_APP_CMD_RECEIVED message.
struct user_app_cmd_received
{
    /// command string legth in bytes
    uint32_t bytes;
    /// command source.
    uint8_t cmd_source;
    /// command buffer
    char cmd_buffer[__ARRAY_EMPTY];
};
#endif 
