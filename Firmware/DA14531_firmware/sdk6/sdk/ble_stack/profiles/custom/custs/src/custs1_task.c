...
#if (!defined (__DA14531__) || defined (__EXCLUDE_ROM_CUSTS1__)) || (defined(__DA14535__) && !defined(__EXCLUDE_ROM_CUSTS1__) && defined(CODELESS_585))
/**
 ****************************************************************************************
...
#else
extern int custs1_att_get_value(uint8_t att_idx, uint16_t *length, const uint8_t **data);
#endif
...
bool custs1_flow_Control_ccc_notifications_enabled(void)
{
    struct custs1_env_tag *custs1_env = PRF_ENV_GET(CUSTS1, custs1);
    uint16_t ccc_hdl;
    uint16_t handle = custs1_env->shdl + CUST1_IDX_FLOW_CONTROL_VAL;
    uint8_t ccc_idx;
    uint8_t status;
    ...
    return (custs1_get_ccc_value(0, ccc_idx) & PRF_CLI_START_NTF);
}
