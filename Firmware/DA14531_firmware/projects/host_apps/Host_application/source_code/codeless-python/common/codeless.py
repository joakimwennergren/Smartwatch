...
#        self.callback_notification_flag = 1
    ...
    def connect(self, baudrate=None, bytesize=None, parity=None, stopbits=None, flowcontrol=None):
        """
        ...
        if self.link_type == CodelessLink.BLE:
            ...
            if (flowcontrol is None) or (flowcontrol == "None"):
                ...
                self.device.xonxoff = False  # disable software flow control
                self.device.rtscts = True  # enable hardware (RTS/CTS) flow control
                self.device.dsrdtr = False  # disable hardware (DSR/DTR) flow control
            elif flowcontrol == "DTR/DSR":
                self.device.xonxoff = False  # disable software flow control
                self.device.rtscts = False  # disable hardware (RTS/CTS) flow control
                self.device.dsrdtr = True  # enable hardware (DSR/DTR) flow control
            # self.device.timeout = None  # block read
            self.device.timeout = 1  # non-block read
            # self.device.writeTimeout = 0  # timeout for write
            # self.device.writeTimeout = 50  # timeout for write
            self.device.writeTimeout = 500  # timeout for write
    ...
    def send_command(self, command_string, timeout_in_sec, carriage_return=True):
        """
            ...
            if self.link_type == CodelessLink.BLE:
                ...
                while self.reply_received == 0:
                    ...
                    if self.reply_string.endswith(termination_list) \
                            or any(ext in self.reply_string for ext in termination_list)\
                            or "OK" in self.reply_string \
                            or "BINREQ" in self.reply_string:
        ...
        pattern = re.compile("(" + "|".join(notification_list) + ")", re.I)
        self.reply_string = pattern.sub("", self.reply_string)
